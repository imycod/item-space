...
export const validators = {
	App: {
		defaultRender(options) {
			...
			cy.validate(
				'HeaderBar',
				extendOptions(options, { testData: { activeItemName: 'Home' } })
			)

			// check some DOM elements directly owned by App
			cy.contains('Pretending to load...').should('not.exist')

			cy.get('img').should('have.attr', 'alt', 'Vue logo').and('have.length', 2)

			// validate the entire component tree of HelloWorld
			cy.validate('HelloWorld', {
				testData: { title: 'Welcome to Your Vue.js App' },
			})
			...
			cy.validate(
				...
				extendOptions(options, {
					testData: {
						activeItemName: 'Some other place',
						isLoggedIn: true,
					},
				})
			)
		...
		defaultRender(options) {
			...
			items.forEach((item) => {
				cy.validate(
					'HelloListItem',
					extendOptions(options, {
						testData: { name: item.name, href: item.href, active: item.active },
					})
				)
			})
		},
	...
	HelloIntro: {
		defaultRender() {
			...
			cy.contains('a', 'vue-cli documentation').should(
				'have.attr',
				'href',
				'https://cli.vuejs.org'
			)
			...
			cy.validate('DisclosureWidget', {
				testData: {
					title: 'For some interactivity, check this out',
					body: 'I am a Disclosure Widget being awesome.',
				},
			})
		},
	},
	...
	HeaderBar: {
		defaultRender(options) {
			...
			const shouldBeActive = items.findIndex(
			...
			if (shouldBeActive === -1) {
				throw new Error(
					`No nav item found with name matching ${activeItemName}`
				)
			}
	...
	OtherPlace: {
		defaultRender() {
			...
			cy.validate('DisclosureWidget', {
				testData: { title: 'Outer Disclosure Title', body: 'Outer body' },
			})
			cy.contains('h2', 'Outer Disclosure Title').click()
			cy.validate('DisclosureWidget', {
				testData: {
					title: 'Inner Disclosure Title',
					body: 'Inner Disclosure Body',
				},
			})
			cy.contains('h3', 'Inner Disclosure Title').click()
			cy.validate('DisclosureWidget', {
				testData: {
					title: 'Inner INNER Disclosure Title',
					body: 'Inner INNER Disclosure Body',
				},
			})
	...
	DisclosureWidget: {
		defaultRender(options) {
			...
			cy.contains('button', title).as('trigger').should('be.visible')
	...
	LoginForm: {
		...
		loginFlow(options) {
			const { username } = options.testData

			requireTruthy('username', options)

			cy.validate('LoginForm', options)
			cy.findByLabelText('Name').type(username)
